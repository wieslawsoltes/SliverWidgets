...
| `SW-FR-017` | `AC-009`, `AC-010`, `AC-011` | Framework gallery samples with the Avalonia reference shell across Avalonia, MAUI, Uno, and WinUI: top metrics header, short scenario tabs, left controls/notes panel, right native viewport, Avalonia header shrink behavior, mixed composition clipping under pinned headers, and configurable section sticky-header behavior with stacked mode as the default without incoming-header blank bands | Sample builds and Avalonia header/mixed-clipping/section-sticky headless tests | Samples docs | Not packaged | Done |
