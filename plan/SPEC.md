...
- `SW-FR-017`: Each supported framework shall provide a gallery-style sample app or sample surface that uses the Avalonia gallery shell as the reference: a top metrics header, the same short scenario tabs (`Fixed`, `Variable`, `Grid`, `Header`, `Tabs`, `Mixed`, `Sections`, `Fill`, `Cache`), a left controls/notes panel, and a right native viewport demonstrating fixed lists, adaptive grids, persistent headers, tabbed nested scroll/overlap concepts, mixed sliver composition without visual overlap through pinned headers, and large-data virtualization.
...
- `AC-010`: Gallery samples include Flutter-inspired examples covering fixed large lists, variable/non-uniform lists, adaptive grids, pinned/collapsible headers, tabbed nested scrolling, sectioned headers, mixed `CustomScrollView` composition, fill/padding/visibility, and cache/performance stress.
...
- Shared gallery scenario data and framework gallery apps: implemented. Avalonia, MAUI, Uno, and WinUI expose the same nine scenario catalog entries through the same top metrics header, short scenario tabs, and left-controls/right-viewport shell. Avalonia, MAUI, Uno, and WinUI compile on macOS; WinUI runtime validation remains a Windows lane.
