...
|---|---|---|---|
| Avalonia | `SliverWidgets.Avalonia` | `Panel`, `Decorator`, `VirtualizingPanel` | `SliverVirtualizingStackPanel`, `SliverVirtualizingListPanel` |
| MAUI | `SliverWidgets.Maui` | `Layout`, `ILayoutManager`, `CollectionView` | `SliverCollectionView` |
| Uno | `SliverWidgets.Uno` | `ItemsRepeater` `VirtualizingLayout` | `SliverFixedExtentVirtualizingLayout`, `SliverGridVirtualizingLayout` |
| WinUI | `SliverWidgets.WinUI` | `ItemsRepeater` `VirtualizingLayout` | `SliverFixedExtentVirtualizingLayout`, `SliverGridVirtualizingLayout` |
...
MAUI does not expose a direct arbitrary item virtualization protocol through `ILayoutManager`; large-data virtualization uses `SliverCollectionView`, which wraps native `CollectionView` recycling.
